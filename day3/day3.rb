...
class Day3 < MiniTest::Test
  ...
  def test_1
    wire_1 = %w[R8 U5 L5 D3]

    wire_2 = %w[U7 R6 D4 L4]

    assert_equal [[3,3], [5,6]], intersection_points(wire_1, wire_2)
    assert_equal 6, closest_distance(wire_1, wire_2)
  end

  def test_2
    wire_1 = %w[R75 D30 R83 U83 L12 D49 R71 U7 L72]

    wire_2 = %w[U62 R66 U55 R34 D71 R55 D58 R83]

    assert_equal 159, closest_distance(wire_1, wire_2)
  end

  def test_3
    wire_1 = %w[R98 U47 R26 D63 R33 U87 L62 D20 R33 U53 R51]

    wire_2 = %w[U98 R91 D20 R16 D67 R40 U7 R15 U6 R7]

    assert_equal 135, closest_distance(wire_1, wire_2)
  end
  ...
  def test_4
    ...
    assert_equal 610, min_signal_delay(wire_1, wire_2)
  ...
  def test_5
    ...
    assert_equal 410, min_signal_delay(wire_1, wire_2)
  end

  def test_final
    wire_1, wire_2 = DATA.each_line.map { |line| line.split(',') }

    assert_equal 375, closest_distance(wire_1, wire_2)
    assert_equal 14746, min_signal_delay(wire_1, wire_2)
  end
  ...
  def closest_distance(wire_1, wire_2)
    points = intersection_points(wire_1, wire_2)
    points.map { |point| point.first.abs - POINT_O.first + point.last.abs - POINT_O.last }.min
  end

  def intersection_points(wire_1, wire_2)
    (make_points(wire_1) & make_points(wire_2)).sort
  end

  def min_signal_delay(wire_1, wire_2)
    points_wire_1 = make_points(wire_1)
    points_wire_2 = make_points(wire_2)
    intersections = (points_wire_1 & points_wire_2).sort
    intersections.map do |point|
      points_wire_1.index(point) + 1 +
        points_wire_2.index(point) + 1
    end.min

  end

  def make_points(wire)
    points = [POINT_O]
    wire.map do |line|
      direction = line[0]
      length = Integer(line[1..-1])
      case direction
      when 'D'
        row = points.last.first - 1
        col = points.last.last
        length.times do |n|
          points << [row - n, col]
        end
      when 'U'
      ...
      when 'L'
        row = points.last.first
        col = points.last.last - 1
        length.times do |n|
          points << [row, col - n]
        end
      when 'R'
