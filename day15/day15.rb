...
class Day15 < MiniTest::Test
  ...
  DROID = DATA.read.split(',').map(&method(:Integer)).freeze

  def test_a_and_b
    assert_equal [228, 348], run_repair_droid(DROID)
  end

  PathSegment = Struct.new(:path, :direction, :int_code_state) do
    # y, x position of the last path element
    def pos
      path.last
    end
  end
  ...
  def run_repair_droid(int_code)
    ...
    all_directions = [WEST, EAST, NORTH, SOUTH]
    oxygen_port_path_length = -1
    output_proc = lambda { |status|
      case status
      when WALL
        y, x = segment.pos
        # direction_s = direction_to_s(segment.direction)
        # puts "Hit the wall #{direction_s} from: #{[y, x]}"
        dead_ends.add([segment.direction, y, x])
      when MOVED_OK
      ...
      when MOVED_ON_TARGET
        if puzzle == :a
          oxygen_port_path_length = segment.path.length
          target_pos = y_x_from(segment)
          # to_y, to_x = y_x_from(segment)
          # puts "Moved On target #{[to_y, to_x]}, path length: #{segment.path.length}"
          int_code.halt!
        end
      else
    ...
    input_proc = lambda {
      ...
      if puzzle == :b
        if segment.nil?
          int_code.halt!
          next NORTH # not important
        end
        # record longest path
        minutes_to_fill = segment.path.length - 1
      end
      # load the int_code_state into the computer
      int_code.state = segment.int_code_state
      # send the direction into the computer
      segment.direction
    }

    start_pos = [0, 0]
    int_code = Intcode.new(int_code, nil, output: output_proc, input: input_proc)
    visited.add(start_pos)
    all_directions.each do |dir|
      queue.push(PathSegment.new([start_pos], dir, int_code.state))
    end
    ...
    # load previous state
    int_code = Intcode.new(int_code.state, nil, output: output_proc, input: input_proc)
    visited.add(target_pos)
    all_directions.each do |dir|
      queue.push(PathSegment.new([target_pos], dir, int_code.state))
    end

    int_code.run

    [oxygen_port_path_length, minutes_to_fill]
  end
  ...
  def y_x_from(segment)
    y, x = segment.path.last
